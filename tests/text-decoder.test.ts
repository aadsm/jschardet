...
import { whatwgDanglingTailWithAsciiPrefix, whatwgDecodesCompletely, whatwgDecodesWithoutError, decoderForLabel } from '../src/text-decoder.js';
...
test('whatwgDanglingTailWithAsciiPrefix: genuine corruption before the tail is false', () => {
  expect(whatwgDanglingTailWithAsciiPrefix('utf-8', new Uint8Array([0xff]))).toBe(false);
});

test('whatwgDecodesCompletely rejects a truncated multi-byte tail', () => {
  ...
  expect(whatwgDecodesWithoutError('utf-8', dangling)).toBe(true);
  expect(whatwgDecodesCompletely('utf-8', dangling)).toBe(false);
