...
- [`pipeline/confusion.py`](https://github.com/chardet/chardet/blob/main/src/chardet/pipeline/confusion.py) builds per-encoding letter/case byte tables at runtime (`_letter_case_table`: codecs + `category()`) for context-aware category voting. Several pair encodings (the EBCDIC pages) have no WHATWG decoder, so the port precomputes the tables at build time by running chardet's own implementation — `src/pipeline/_confusion-case-tables.ts`, generated by [`scripts/generate-confusion-case-tables.js`](../scripts/generate-confusion-case-tables.js).
