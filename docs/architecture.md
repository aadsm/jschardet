...
However, some SBCS encodings (windows-125x and others) leave certain byte positions undefined — Python treats those bytes as errors, but WHATWG `TextDecoder` silently accepts them because the WHATWG spec fills those gaps. Single-byte encodings therefore never reach `TextDecoder` in the validity stage: [`src/pipeline/byte-decode.ts`](../src/pipeline/byte-decode.ts) answers them from the build-time byte tables (`_byte-decode-tables.ts`, see e. below), where an undefined position carries the code-point sentinel, and rejects any candidate whose undefined bytes appear in the input, matching Python's stricter behaviour. The one exception is `ascii`, deliberately kept on its WHATWG alias (windows-1252) in the tolerant predicate.
