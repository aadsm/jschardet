...
- **EBCDIC** — the markup stage scans a cp037 decode of the head for charset declarations, using a byte→character table baked into [`src/pipeline/markup.ts`](../src/pipeline/markup.ts) from Python's codec; the declared MAINFRAME-era name's own decode check reduces, for a single-byte encoding, to "contains no undefined byte", answered from `sbcs-undefined-bytes.ts`.
...
- [`pipeline/confusion.py`](https://github.com/chardet/chardet/blob/main/src/chardet/pipeline/confusion.py) builds per-encoding letter/case byte tables at runtime (`_letter_case_table`: codecs + `category()`) for context-aware category voting, and (for pairs the confusion maps do not cover) decodes single bytes under arbitrary encodings with `codecs` and reads their `unicodedata.category()` in `differing_high_bytes` / `_pair_categories`. Several of these encodings (the EBCDIC pages, hp-roman8, koi8-t) have no WHATWG decoder, so the port precomputes the byte data at build time by running Python over every encoding in the registry — `src/pipeline/_byte-decode-tables.ts` (each byte's decoded general category, and its code point for the high bytes), generated by [`scripts/generate-byte-tables.js`](../scripts/generate-byte-tables.js). The letter/case table is derived from those categories at runtime rather than stored separately (`_letter_case_table`'s 0/1/2 verdict is a pure function of the general category; the generator asserts the derivation matches chardet's own table before emitting).
