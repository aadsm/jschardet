...
**TypeScript:** `decodesWithoutError` in `src/pipeline/validity.ts` is the port, keyed by chardet name; single-byte encodings are answered from the byte tables, and the multi-byte encodings reach the same tail tolerance through `whatwgDecodesWithoutError` in `src/text-decoder.ts`, which decodes by WHATWG label with `{ stream: true }`. `filterByValidity` goes through the port; `_validateBytes` and `promoteMarkupSuperset` call the WHATWG helper directly.
