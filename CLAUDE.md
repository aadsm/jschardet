...
- `scripts/generate-confusion-case-tables.js` — generates `src/pipeline/_confusion-case-tables.ts` (per-encoding letter/case byte tables for confusion category voting) by running chardet's `confusion._letter_case_table` over the pinned submodule; called automatically by `update-chardet` after each pin change
