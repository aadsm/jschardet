...
- `scripts/generate-byte-tables.js` — generates `src/pipeline/_byte-decode-tables.ts` for every encoding in chardet's registry: per single byte, its decoded Unicode general category (all 256) and, for the high bytes (0x80–0xFF), its decoded code point, computed by Python's `codecs` + `unicodedata`. The table backs `confusion.ts`'s `differingHighBytes` / `_pairCategories` (which arbitrate encoding pairs the confusion maps do not cover) and `_letterCaseTable`, whose letter/case verdict is derived from the stored categories (the generator asserts the derivation matches chardet's own `confusion._letter_case_table` byte-for-byte before emitting, so the letter/case verdict needs no table of its own). Called automatically by `update-chardet` after each pin change. See `docs/textdecoder-vs-python.md` for why this table exists — one of several bridges over the TextDecoder-vs-CPython gap
