...
export function isBinary(data, maxBytes = DEFAULT_MAX_BYTES) {
    ...
    for (const b of data) {
        ...
        if (b === _EBCDIC_SPACE || b === _EBCDIC_HT)
            spaceCount++;
        if (b >= 0x80)
            highCount++;
    }
    if (binaryCount / data.length <= _BINARY_THRESHOLD)
        return false;
    ...
    if (hardCount / data.length > _BINARY_THRESHOLD)
        return true;
    const nonSpace = data.length - spaceCount;
    if (nonSpace === 0)
        return false;
    if (highCount / nonSpace < _EBCDIC_MIN_HIGH_FRACTION)
        return true;
