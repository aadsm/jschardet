...
function _brandKey(data, offset) {
        ...
        if (name.length === mimetypeBytes.length && startsWith(name, mimetypeBytes)) {
            ...
            if (compression === 0) { // stored (uncompressed)
                ...
                if (scan.length >= contentStart + contentLen) {
                    ...
                    if (_OPENDOCUMENT_MIMES.has(contentStr))
                        return contentStr;
                }
            }
        }
...
export function detectMagic(data) {
    ...
    if (data.length >= 12 && startsWith(data.subarray(_FTYP_OFFSET), _FTYP_MARKER)) {
        ...
        if (boxSize >= 8 && boxSize <= data.length) {
            ...
            if (_FTYP_AVIF_BRANDS.has(brand))
                return _makeResult('image/avif');
            if (_FTYP_HEIC_BRANDS.has(brand))
                return _makeResult('image/heic');
            if (_FTYP_HEIF_BRANDS.has(brand))
                return _makeResult('image/heif');
            if (_FTYP_AUDIO_BRANDS.has(brand))
                return _makeResult('audio/mp4');
            if (_FTYP_QT_BRANDS.has(brand))
                return _makeResult('video/quicktime');
    ...
    if (data.length >= 8 && startsWith(data, _CAFEBABE)) {
        ...
        if (nfatArch <= _CAFEBABE_MAX_FAT_ARCHES)
            return _makeResult('application/x-mach-binary');
