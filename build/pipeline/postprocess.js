...
import { ART_LANGUAGE, ASCII_WHITESPACE_TABLE, BigramProfile, RARE_LANGUAGES, getEncIndex, getIdfWeights, scoreWithProfile, } from '../models/index.js';
import { REGISTRY, lookupEncoding } from '../registry.js';
import { _COMPAT_NAMES } from '../output_names.js';
import { danglingTailWithAsciiPrefix, decodesCompletely, decodesWithoutError, whatwgLabelFor, } from '../text-decoder.js';
...
const _COMMON_LATIN_ENCODINGS = new Set([
    'iso8859-1',
    'iso8859-15',
    'cp1252',
]);
...
const _ISO_8859_10_DISTINGUISHING = new Set([
...
const _ISO_8859_14_DISTINGUISHING = new Set([
...
const _WINDOWS_1254_DISTINGUISHING = new Set([
...
const _HP_ROMAN8_DISTINGUISHING = new Set([
...
const _DEMOTION_CANDIDATES = new Map([
...
// KOI8-T over KOI8-R.
const _KOI8_T_DISTINGUISHING = new Set([
...
function _shouldDemote(encoding, data, language) {
...
function _demoteNicheLatin(data, results) {
    if (results.length > 1
        ...
        // and stays put.
        const candidates = results.slice(1).filter((x) => x.encoding !== null && _COMMON_LATIN_ENCODINGS.has(x.encoding));
        if (candidates.length > 0) {
            const leadConf = candidates[0].confidence;
            const inBand = candidates.filter(x => leadConf - x.confidence <= _DEAD_HEAT_EPSILON);
            let r = inBand[0];
            ...
            const others = results.filter(x => x.encoding !== demotedEncoding && x !== r);
...
function _promoteKoi8t(data, results) {
...
const _DEAD_HEAT_SUPERSETS = Object.freeze({
...
function _eraRank(encoding) {
    const info = REGISTRY[encoding];
    if (info === undefined)
        return 1 << 30;
    const era = info.era;
    return era & -era;
...
// path.
function _hasHighByteEvidence(data, encoding, language) {
...
function _highByteEvidenceMargin(data, encoding, language) {
    const variants = getEncIndex().get(encoding);
    if (variants === undefined || variants.length === 0)
        return 0;
    const window = data.subarray(0, _EVIDENCE_SCAN_MAX_BYTES);
...
// small its margin.
function _preferPrevalentOnDeadHeat(data, results) {
    const top = results.length > 0 ? results[0] : null;
    if (top === null || top.encoding === null || results.length < 2)
        return results;
...
function _promoteToTop(results, i) {
    const r = results[i];
    const promoted = {
        encoding: r.encoding,
...
function _promoteSupersetOnDeadHeat(data, results) {
    const top = results.length > 0 ? results[0] : null;
    if (top === null || top.encoding === null || results.length < 2)
        return results;
...
function _arbitrateRareLanguage(results) {
    const top = results.length > 0 ? results[0] : null;
    if (top === null ||
        top.encoding === null ||
        top.language === null ||
    ...
    for (let i = 1; i < results.length; i++) {
        ...
        if (top.confidence - r.confidence > _RARE_ARBITRATION_MARGIN)
            break;
        if (r.encoding === null || r.language === null)
            continue;
        if (!RARE_LANGUAGES.has(r.language))
            return _promoteToTop(results, i);
    }
    return results;
}
...
function _promoteMacOnCrLineEndings(data, results) {
    const top = results.length > 0 ? results[0] : null;
    if (top === null || top.encoding === null || results.length < 2)
        return results;
    if (_eraRank(top.encoding) === _LEGACY_MAC_ERA)
        return results;
...
function _decodesUnderPublicNames(data, encoding) {
    const label = whatwgLabelFor(encoding);
    if (label === null || !decodesCompletely(label, data))
        return false;
...
function _preferDecodableOnTie(data, results, inputTruncated) {
    const top = results.length > 0 ? results[0] : null;
    ...
    for (let i = 1; i < results.length; i++) {
        const r = results[i];
        if (r.encoding === null || !_decodesUnderPublicNames(data, r.encoding))
            continue;
        return _promoteToTop(results, i);
    }
    return results;
...
export function postprocessResults(data, results, { inputTruncated = false } = {}) {
...
export { _COMMON_LATIN_ENCODINGS, _DEMOTION_CANDIDATES, _HP_ROMAN8_DISTINGUISHING, _ISO_8859_10_DISTINGUISHING, _ISO_8859_14_DISTINGUISHING, _KOI8_T_DISTINGUISHING, _WINDOWS_1254_DISTINGUISHING, _arbitrateRareLanguage, _decodesUnderPublicNames, _demoteNicheLatin, _hasHighByteEvidence, _highByteEvidenceMargin, _preferDecodableOnTie, _preferPrevalentOnDeadHeat, _promoteKoi8t, _promoteMacOnCrLineEndings, _promoteSupersetOnDeadHeat, _shouldDemote, };
