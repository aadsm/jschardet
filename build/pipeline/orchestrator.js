...
import { _NONE_RESULT, DETERMINISTIC_CONFIDENCE, PipelineContext, } from './index.js';
...
import { computeLeadByteDiversity, computeMultibyteByteCoverage, computeStructuralScore, } from './structural.js';
import { detectUtf8 } from './utf8.js';
import { detectUtf1632Patterns } from './utf1632.js';
import { filterByValidity } from './validity.js';
import { getCandidates } from '../registry.js';
...
const _BINARY_RESULT = Object.freeze({
    encoding: null,
...
function _makeFallbackOrNone(encoding, allowed, paramName) {
    if (!allowed.has(encoding)) {
        ...
        console.warn(`${paramName} '${encoding}' is excluded by include_encodings/exclude_encodings; returning encoding=None`);
        return [{ ..._NONE_RESULT }];
    }
...
function _gateCjkCandidates(data, validCandidates, ctx) {
    ...
    for (const enc of validCandidates) {
        if (enc.isMultibyte) {
            ...
            if (ctx.nonAsciiCount < _CJK_MIN_NON_ASCII)
                continue; // Too few high bytes to trust the score
            const byteCoverage = computeMultibyteByteCoverage(data, enc, ctx, ctx.nonAsciiCount);
            ctx.mbCoverage.set(enc.name, byteCoverage);
            if (byteCoverage < _CJK_MIN_BYTE_COVERAGE)
            ...
            if (ctx.nonAsciiCount >= _CJK_DIVERSITY_MIN_NON_ASCII) {
                ...
                if (leadDiversity < _CJK_MIN_LEAD_DIVERSITY)
...
function _scoreStructuralCandidates(data, structuralScores, validCandidates, ctx) {
    const encLookup = new Map();
    for (const e of validCandidates) {
        if (e.isMultibyte)
            encLookup.set(e.name, e);
    }
    const validMb = [];
    ...
    const results = scoreCandidates(data.subarray(0, _STAT_SCORE_MAX_BYTES), [...validMb, ...singleByte]);
...
function _runPipelineCore(data, encodingEra, maxBytes, includeEncodings, excludeEncodings, noMatchEncoding, emptyInputEncoding, inputTruncated) {
    ...
    const candidates = getCandidates(encodingEra, includeEncodings ?? undefined, excludeEncodings ?? undefined);
    const allowed = new Set(candidates.map(enc => enc.name));
    if (data.length === 0) {
    ...
    if (structuralScores.length > 0) {
        ...
        if (bestScore >= _STRUCTURAL_CONFIDENCE_THRESHOLD) {
            const results = _scoreStructuralCandidates(data, structuralScores, validCandidates, ctx);
            if (results.length > 0) {
...
export function runPipeline(data, encodingEra, options) {
    ...
    let results = _runPipelineCore(data, encodingEra, maxBytes, includeEncodings, excludeEncodings, noMatchEncoding, emptyInputEncoding, options?.inputTruncated ?? false);
    ...
    results = results.map(r => r.language === ART_LANGUAGE
    ...
    return results.map(r => r.confidence > 1.0
...
export { _BINARY_RESULT, _gateCjkCandidates, _makeFallbackOrNone, _runPipelineCore, _scoreStructuralCandidates, _withDefaultMime, };
