...
import { ART_LANGUAGE, BigramProfile, getEncIndex, getIdfWeights, scoreWithProfile, } from '../models/index.js';
...
export function _deserializeConfusionDataFromBytes(data) {
...
let cached = null;
export function loadConfusionMaps() {
    if (cached)
        return cached;
    const raw = readConfusionBin();
    if (raw.length === 0) {
        console.warn('jschardet confusion.bin is empty — confusion resolution disabled; ' +
...
const _caseTableCache = new Map();
let _caseTablesByCanonical = null;
function _letterCaseTable(encoding) {
    ...
    if (digits === undefined) {
        ...
        for (let i = 0; i < 256; i++)
            table[i] = digits.charCodeAt(i) - 0x30;
    }
...
function _contextPreference(cat, left, right, caseTable) {
...
const _membershipCache = new WeakMap();
function _isDiffTable(diffBytes) {
    let table = _membershipCache.get(diffBytes);
    if (table === undefined) {
        table = new Uint8Array(256);
        for (const bv of diffBytes)
            table[bv] = 1;
...
// one loud byte.
function _voteWithMargin(data, encA, encB, diffBytes, categories) {
...
export function confusionPairWinner(data, encX, encY, languages = new Set()) {
    ...
    if (diffBytes.size >= _CROSS_FAMILY_MIN_DIFFS) {
        ...
        if (bigramWinner !== null && bigramWinner === vote.winner)
            return bigramWinner;
        return null;
    }
...
export function resolveByCategoryVoting(data, encA, encB, diffBytes, categories) {
    return _voteWithMargin(data, encA, encB, diffBytes, categories).winner;
}
const _modelledLanguagesCache = new Map();
// Languages enc has a bigram model for (chardet's _modelled_languages).
export function _modelledLanguages(enc) {
...
export function _comparableLanguages(encA, encB, languages) {
    if (languages.size === 0)
        return null;
...
export function _bestVariantScore(profile, enc, languages) {
    const variants = getEncIndex().get(enc);
    if (variants === undefined || variants.length === 0)
        return 0.0;
    let best = 0.0;
    for (const [lang, model, modelKey] of variants) {
        if (languages !== null && (lang === null || !languages.has(lang)))
            continue;
...
export function resolveByBigramRescore(data, encA, encB, diffBytes, languages = new Set()) {
    if (data.length < 2)
        return null;
...
function _findPairKey(maps, encA, encB) {
    if (maps.has(pairKey(encA, encB)))
        return [encA, encB];
    if (maps.has(pairKey(encB, encA)))
        return [encB, encA];
    return null;
}
...
export function resolveConfusionGroups(data, results) {
    if (results.length < 2)
        return results;
    const top = results[0];
    ...
    let championIdx = 0;
    let champion = top;
    let championEnc = top.encoding;
    for (let i = 1; i < results.length; i++) {
        const candidate = results[i];
        if (candidate.encoding === null)
            continue;
        ...
        if (vote.winner !== null &&
            ...
            const langs = new Set();
            if (champion.language !== null)
                langs.add(champion.language);
            if (candidate.language !== null)
                langs.add(candidate.language);
