import { DETERMINISTIC_CONFIDENCE, PipelineContext } from './index.js';
import { lookupEncoding, REGISTRY } from '../registry.js';
...
function _validateBytes(data, encoding) {
...
function _decodesAsMainframe(head, encoding) {
    const label = whatwgLabelFor(encoding);
    if (label !== null)
        return decodesWithoutError(label, head);
    const undefSet = SBCS_UNDEFINED_BYTES[encoding];
    if (undefSet === undefined)
        return true;
    for (let i = 0; i < head.length; i++) {
        if (undefSet.has(head[i]))
            return false;
    }
    return true;
}
...
function _detectEbcdicDeclaration(head) {
    let highCount = 0;
    for (let i = 0; i < head.length; i++) {
        if (head[i] >= 0x80)
            highCount++;
    }
    if (highCount < head.length * _EBCDIC_SCAN_MIN_HIGH_FRACTION)
        return null;
    let decoded = '';
    for (let i = 0; i < head.length; i++)
        decoded += _CP037_TABLE[head[i]];
...
function _detectPep263(data) {
    ...
    if (encoding === null || !_validateBytes(data, encoding))
        return null;
    return {
        encoding,
        confidence: DETERMINISTIC_CONFIDENCE,
...
export function detectMarkupCharset(data) {
    if (data.length === 0)
        return null;
    ...
    for (const [re, mimeType] of patterns) {
        ...
        if (encoding === null)
            continue;
        if (!_validateBytes(data, encoding))
            continue;
...
const _MARKUP_SUPERSET_PROMOTIONS = Object.freeze({
    shift_jis_2004: 'cp932',
    euc_kr: 'cp949',
});
export function promoteMarkupSuperset(data, markupResult, allowed) {
