import { ASCII_TEXT_BYTES, DETERMINISTIC_CONFIDENCE } from './index.js';
...
function _isNullSeparatorPattern(data, nullFrac) {
    if (nullFrac >= _NULL_SEPARATOR_MAX_FRACTION)
        return false;
    // Replaces Python bytes.translate(None, ALLOWED) — count bytes not in the allowed set
    for (const b of data) {
        if (!_NULL_SEPARATOR_ALLOWED.has(b))
            return false;
    }
    return true;
}
export function detectUtf1632Patterns(data) {
    const sample = data.subarray(0, _SAMPLE_SIZE);
    if (sample.length < _MIN_BYTES_UTF16)
        return null;
...
function _checkUtf32(data) {
    const trimmedLen = data.length - (data.length % 4);
    if (trimmedLen < _MIN_BYTES_UTF32)
        return null;
...
export function _looksLikeText(text) {
    if (!text)
        return false;
    let total = 0;
    let printable = 0;
    for (const c of text) {
        ...
        if (!_RE_NONPRINTABLE.test(c))
            printable++;
    }
    return printable / total > _MIN_PRINTABLE_FRACTION;
}
export function _textQuality(text, limit = 500) {
    let n = 0;
    let letters = 0;
    let marks = 0;
    let spaces = 0;
...
//# sourceMappingURL=utf1632.js.map