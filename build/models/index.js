...
export function _parseModelsBin(data) {
    if (data.length < 4 ||
    ...
    try {
        ...
        if (blob.length !== expectedSize) {
            throw new Error(`corrupt models.bin: blob size ${blob.length} != expected decompressed size ${expectedSize}`);
        }
...
function loadModelData() {
    ...
    if (data.length === 0) {
        console.warn('jschardet models.bin is empty — statistical detection disabled; ' +
...
// Exported with underscore prefix as an internal helper for the test suite.
export function _buildEncIndex(models) {
    const index = new Map();
...
let encIndexCache = null;
export function getEncIndex() {
    if (encIndexCache)
        return encIndexCache;
    encIndexCache = _buildEncIndex(loadModels());
    return encIndexCache;
}
export function inferLanguage(encoding) {
    return SINGLE_LANG_MAP[encoding] ?? null;
}
export function hasModelVariants(encoding) {
    return getEncIndex().has(encoding);
}
let idfWeightsCache = null;
export function getIdfWeights() {
    if (idfWeightsCache)
        return idfWeightsCache;
    const data = readIdfBin();
    if (data.length !== 65536) {
        console.warn(`jschardet idf.bin has wrong size (${data.length}), ` +
...
export class BigramProfile {
    ...
    constructor(data) {
        ...
        for (let i = 0; i < totalBigrams; i++) {
            ...
            if (b1 === b2 && ASCII_WHITESPACE_TABLE[b1])
                continue;
            const idx = (b1 << 8) | b2;
            const w = idf[idx];
            if (freq[idx] === 0)
    ...
    static fromWeightedFreq(weightedFreq) {
...
export function scoreWithProfile(profile, model, modelKey = '') {
    if (profile.inputNorm === 0)
        return 0;
...
export const RARE_LANGUAGES = new Set(['gd', 'cy', 'ga', 'br']);
...
export function scoreBestLanguage(data, encoding, profile, { demoteThinRare = false } = {}) {
    if (data.length === 0 && profile === undefined)
        return [0, null];
